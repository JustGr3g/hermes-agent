...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        result = (
            "## Skills\n"
            "Skills package proven workflows, real tool commands, and the quality "
            "bar for recurring tasks. Load one with skill_view(name), then follow "
            "it. Don't pre-scan the catalog every turn — load when a trigger below "
            "fires, or when a task clearly maps to a catalog skill.\n"
            "\n"
            "Load a skill when:\n"
            "- You're about to finalize code changes you intend to keep → "
            "`requesting-code-review`.\n"
            "- Starting a new feature or a multi-file change → `writing-plans` "
            "(plan first, then build).\n"
            "- Debugging a failure whose cause isn't obvious → "
            "`systematic-debugging`.\n"
            "- Writing or modifying tests → `test-driven-development`.\n"
            "- Delegating work to a subagent → `subagent-driven-development`.\n"
            "- Configuring/troubleshooting Hermes Agent itself — CLI, config, "
            "models, providers, tools, gateway, plugins → `hermes-agent` (it has "
            "the real commands).\n"
            "- A task clearly maps to a catalog skill below — e.g. Obsidian vault "
            "work → `obsidian`, GitHub PR work → `github-pr-workflow`.\n"
            "\n"
            "If a skill you load is outdated or wrong, patch it with "
            "skill_manage(action='patch'). After a complex task (5+ tool calls) "
            "or a reusable workflow you discovered, save it with skill_manage.\n"
            "\n"
            "<available_skills>\n"
            + "\n".join(index_lines) + "\n"
            "</available_skills>"
