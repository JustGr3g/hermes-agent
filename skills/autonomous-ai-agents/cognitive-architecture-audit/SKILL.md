...
- `references/surprise-to-behavior-pipeline-gap-jun5.md` — Second iteration of the inner-speech-as-shaping audit (2026-06-05). Mapped the six-stage surprise signal lifecycle, identified the missing stage 5, shipped the SpeechType.SURPRISE + MetaSignal.SURPRISE_DETECTED + Rule 4e triplet via subagent, plus the audit-trail fix for `_record_synth_outcome`. Includes the v2 follow-up gap class (trajectory signatures, recovery loop, tone aliasing, Simulator coupling) for the next iteration.
- `references/novel-concept-learning-implementation-jun5.md` — End-to-end novel-concept learning flow: Greg's introspection question → in-conversation diagnosis → 3-gap pen with file:line citations → 4-file structural implementation → 16/16 + 9/9 verification gate. Worked example of the "deterministic contract over LLM self-assessment" technique. Pattern-match when an audit surfaces "the LLM is being asked to evaluate itself and consistently gets it wrong."
