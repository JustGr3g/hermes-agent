...
- **Don't trust stale roadmap documents over live disk state.** A plan from May 14 may describe gaps that have since been filled. Before claiming a file, class, or function is missing, `search_files()` for the exact name (class name, function name, file path) in the target tree. If the search finds it, the gap claim is wrong — update your mental model, don't write it into the audit. The plan document is a historical snapshot; disk state is truth. This applies especially when the plan claims "subsystem X references class Y that doesn't exist on disk" — grep for `class Y` before repeating the claim.
...
- **Check model-level fields even when they aren't referenced at the detection site.** When auditing a decision boundary (e.g. "is this goal complete?"), it's tempting to only trace what feeds into the explicit detection code — `goal.content`, `tool_name`, `result` — and not check whether the source data model has additional fields the detector could be using but isn't. The goal's `success_criteria` field exists on the `Goal` dataclass (motivation.py:131), is populated by the proposer (~200 DB rows), but is read by zero completion detectors. This pattern repeats across audit targets: a model carries a field, but the consuming subsystem never touches it. **Trace the full data model at the source, not just what the consumer references.**
- **Use `wc -l` for test file sizes, `grep -c` for test count.** Don't guess coverage — measure it.
- **For "is X load-bearing?" questions, also check whether the taxonomy can even express X.** When auditing a tag-to-behavior mapping (e.g. "does surprise change actions?"), the answer is sometimes "no — and the taxonomy has no `SURPRISE` entry at all, so the question was malformed from the start." Check the enum/type definition first; if the label you want to trace doesn't exist there, the audit finds its real answer in the enum file, not the consumer chain. A failed Phase 1 lookup is still a finding.
