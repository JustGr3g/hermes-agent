...
def _get_backend() -> str:
    ...
    if configured in {"parallel", "firecrawl", "tavily", "exa", "searxng", "brave-free", "ddgs", "ollama", "xai"}:
        return configured
...
def web_search_tool(query: str, limit: int = 5) -> str:
    ...
    try:
        ...
        # Dispatch through the web search registry. All 7 upstream providers
        # (brave-free, ddgs, searxng, exa, parallel, tavily, firecrawl)
        # now live as plugins; the dispatcher is just a registry lookup +
        # delegation. Sync only — every provider's search() is sync.
        #
        # MERGE NOTE 2026-05-30: upstream v0.15.0 replaced the per-backend
        # if/elif dispatch with this registry pattern. Athena's local
        # ``ollama`` backend (tools/web_providers/ollama_search.py exists
        # but wasn't registered) needs to be wrapped as a registered plugin
        # under plugins/web/ to be reachable here. Until then, configuring
        # ``backend: ollama`` falls back to ``get_active_search_provider()``.
