...
class AIAgent:
    ...
    def __init__(
        ...
        # ── ATHENA CognitiveProcessor ──────────────────────────────────
        # Lazy-import to avoid hard dependency when cognitive systems are absent.
        # Phase 1 (shell): processor is a no-op pass-through.
        # Phase 3+: full 8-system cognitive loop.
        self._cog = None
        if cognitive_processor is not None:
            self._cog = cognitive_processor
            # Wire the introspection tool to this processor so the LLM can
            # query its cognitive state mid-turn via the ``introspect`` tool.
            try:
                ...
                logger.debug(
                    "[ATHENA] Could not wire introspection tool", exc_info=True
                )
            logger.info("[ATHENA] CognitiveProcessor attached to AIAgent")
        elif os.environ.get("ATHENA_GATEWAY_DELEGATE", "").lower() in (
            "1",
            "true",
            "yes",
        ):
            # Athena's gateway delegates cognition to the sidecar
            # (athena_server.py on :8765). The gateway keeps the LLM call
            # (preserving streaming and matching native Hermes latency);
            # the sidecar owns the singleton CognitiveAgent. RemoteCognitiveProcessor
            # is a thin HTTP shim that mirrors CognitiveProcessor's interface
            # — augment_message / record_result / record_completion /
            # verify_and_revise — so the rest of run_agent.py needs no
            # awareness of the indirection.
            try:
                from agent.remote_cognitive_processor import RemoteCognitiveProcessor

                self._cog = RemoteCognitiveProcessor()
                logger.info(
                    "[ATHENA] ATHENA_GATEWAY_DELEGATE=1 — RemoteCognitiveProcessor "
                    "wired (cognition owned by sidecar at :8765)."
                )
            except Exception as _remote_err:
                logger.warning(
                    "[ATHENA] RemoteCognitiveProcessor init failed (%s); "
                    "gateway will run without cognition (vanilla Hermes fallback).",
                    _remote_err,
                )
        else:
            try:
                ...
                _athena_cog_path = Path(__file__).parent.parent / "cognitive_agent"
                if _athena_cog_path.exists():
                    # Wire the fast client from ATHENA_FAST_MODEL when set, so
                    # cognitive subsystems (saliency / inner-speech / predictive
                    # / tool-select inside ca.run()) actually use a fast LLM
                    # instead of falling back to template/heuristic paths.
                    # Mirrors athena_server.py:_load_agent.
                    _fast_ollama = None
                    _fast_model_env = os.environ.get("ATHENA_FAST_MODEL")
                    if _fast_model_env:
                        try:
                            # The project root needs to be on sys.path before
                            # we can `from cognitive_agent.ollama_client …`.
                            # cognitive_processor._init_systems does this too,
                            # but we run earlier than that.
                            _proj_root_for_fast = (
                                _athena_cog_path.parent  # = cognitive-agent/
                            )
                            if str(_proj_root_for_fast) not in sys.path:
                                sys.path.insert(0, str(_proj_root_for_fast))
                            from cognitive_agent.ollama_client import (
                                OllamaClient as _OllamaC,
                            )

                            _fast_base_url = os.environ.get(
                                "ATHENA_FAST_BASE_URL", "http://localhost:11434"
                            )
                            # Local daemon proxies :cloud models with no key;
                            # only direct ollama.com requires the API key.
                            _need_key = "ollama.com" in _fast_base_url
                            _fast_api_key = None
                            if _need_key:
                                try:
                                    from cognitive_agent.ollama_client import (
                                        get_api_key as _get_athena_key,
                                    )

                                    _fast_api_key = _get_athena_key()
                                except Exception:
                                    _fast_api_key = None
                            _fast_ollama = _OllamaC(
                                base_url=_fast_base_url,
                                api_key=_fast_api_key or "",
                                model=_fast_model_env,
                            )
                            logger.info(
                                "[ATHENA] Fast client wired: model=%s base_url=%s",
                                _fast_model_env,
                                _fast_base_url,
                            )
                        except Exception as _fast_err:
                            logger.warning(
                                "[ATHENA] Fast client init failed (%s) — "
                                "subsystems will use template fallbacks",
                                _fast_err,
                            )
                            _fast_ollama = None
                    self._cog = CognitiveProcessor(
                        fast_ollama_client=_fast_ollama, session_id=self.session_id
                    )
                    logger.info(
                        "[ATHENA] CognitiveProcessor auto-initialized (fast=%s)",
                        "on" if _fast_ollama else "off",
                    )

                    # Phase 2A: load stored cognitive bridge state from prior turn
                    if self._session_db and self.session_id:
                        try:
                            _stored = self._session_db.get_cognitive_state(
                                self.session_id
                            )
                            if _stored:
                                self._cog.load_cog_state(_stored)
                        except Exception as _cse:
                            logger.debug(
                                "[ATHENA] Could not load stored cog state: %s", _cse
                            )
            except Exception as _cog_err:
                logger.debug("[ATHENA] CognitiveProcessor not available: %s", _cog_err)
        ...
        # Save unfiltered tools for cognitive branching restoration
        self._unfiltered_tools = list(self.tools) if self.tools else []
        ...
        self._memory_nudge_interval = 10  # baseline fallback; Phase 2B doubles this to 20 for timer path since signal-driven extraction now catches important events
    ...
    def _summarize_api_error(error: Exception) -> str:
        ...
        1. Cloudflare/proxy HTML error pages — pulls <title> and Ray ID.
        2. Upstream 5xx with a support ref UUID (e.g. Ollama Cloud's
           ``{"error": "Internal Server Error (ref: <uuid>)"}``) — rewrites
           the raw infrastructure-internals string as a friendly
           "upstream hiccup" message while preserving the ref for ops.
        3. JSON body errors from OpenAI/Anthropic-shaped SDKs where
           ``body["error"]`` is either a dict ``{"message": "..."}`` or
           a plain string.

        Falls back to a truncated ``str(error)`` for everything else.
        """
        raw = str(error)
        ...
        # 2026-05-21: upstream 5xx with a support ref UUID. Observed in the
        # wild from Ollama Cloud as ``{'error': 'Internal Server Error
        # (ref: 58d8c38a-...)'}``. The raw ``str(error)`` form was leaking
        # to Telegram users as "Error code: 500 - {'error': 'Internal
        # Server Error (ref: ...)'}", which reads as scary infra noise.
        # Keep the ref UUID (support needs it) but rewrite the surrounding
        # framing. Match before the JSON-body branch since this catches
        # the string-valued ``body["error"]`` shape that branch was
        # silently falling through.
        if status_code and 500 <= int(status_code) < 600:
            ref_match = re.search(
                r"ref:\s*([A-Fa-f0-9][A-Fa-f0-9-]{6,})", raw
            )
            ref_part = f" (ref: {ref_match.group(1)})" if ref_match else ""
            return (
                f"Upstream API hiccup (HTTP {status_code}). "
    ...
    def _compress_context(
        ...
        *,
        ...
        # Phase 3C: save cognitive context before compression wipes the session
        if self._cog:
            try:
                cs = self._cog.cognitive_state
                pre_compress = {
                    "inner_speech_arc": cs.get("inner_speech_winner", ""),
                    "inner_speech_arc_raw": cs.get("inner_speech_arc", []),
                    "confidence": cs.get("confidence", 0.5),
                    "metacognitive_signals": cs.get("metacognitive_signals", []),
                    "active_goals": cs.get("active_goals", []),
                    "timestamp": time.time(),
                }
                self._cog._last_pre_compress_state = pre_compress

                # Tier 3: save pre-compression decision trace to episodic memory
                _ca = getattr(self._cog, "_systems", {}).get("cognitive_agent")
                if _ca is not None and hasattr(_ca, "episodic"):
                    try:
                        from cognitive_agent.memory.episodic import Episode as _Ep
                        _trace_ep = _Ep(
                            id=str(uuid.uuid4()),
                            session_id=self.session_id or "compress",
                            timestamp=time.time(),
                            content=(
                                f"Pre-compression state: confidence={cs.get('confidence', 0.5):.0%}, "
                                f"failure_rate={cs.get('failure_rate', 0.0):.0%}, "
                                f"wm_load={cs.get('wm_load', 0.0):.0%}"
                            ),
                            summary="Pre-compression decision trace",
                            emotional_tags={
                                "source_kind": _Ep.SOURCE_SYSTEM,
                                "pre_compression": "1",
                                "confidence": str(round(cs.get("confidence", 0.5), 3)),
                                "failure_rate": str(round(cs.get("failure_rate", 0.0), 3)),
                                "wm_load": str(round(cs.get("wm_load", 0.0), 3)),
                            },
                            importance=0.3,
                            terminal=False,
                        )
                        _ca.episodic.store(_trace_ep)
                        # Also store assistant message contents for reasoning continuity
                        _asst_msgs = [
                            m.get("content", "")[:200]
                            for m in messages[-8:]
                        ...
                        if _asst_msgs:
                            _reasoning_ep = _Ep(
                                ...
                                session_id=self.session_id or "compress",
                                timestamp=time.time(),
                                content=" | ".join(_asst_msgs[-5:]),
                                summary="Pre-compression assistant reasoning",
                                emotional_tags={
                                    "source_kind": _Ep.SOURCE_SYSTEM,
                                    "pre_compression": "reasoning",
                                },
                                importance=0.35,
                                terminal=False,
                            )
                            _ca.episodic.store(_reasoning_ep)
                    except Exception:
                        pass
            except Exception:
                pass
    ...
    # ── Cognitive tool enforcement (Tier 1) ────────────────────────────────

    def _apply_cognitive_tool_filter(self) -> None:
        """Strip high-blast-radius tools when cognitive signals indicate distress.

        Operates on self.tools in-place. Never raises — on any error, restores
        the full unfiltered set. Safe to call multiple times per turn.
        """
        if not hasattr(self, "_unfiltered_tools") or not self._unfiltered_tools:
            return
        try:
            _WRITE_TOOLS = frozenset({
                "write_file", "patch", "terminal", "code_execution",
                "save_note", "send_message",
            })
            _RISKY_TOOLS = frozenset({
                "terminal", "code_execution",
            })

            # Start from the unfiltered baseline each time
            filtered = list(self._unfiltered_tools)

            # Repeated failure or WM overloaded: strip write/side-effect tools
            if self._repeated_failure or self._wm_overloaded:
                filtered = [
                    t for t in filtered
                    if t.get("function", {}).get("name") not in _WRITE_TOOLS
                ]
                logging.info(
                    "[ATHENA] Cognitive filter: stripping write tools "
                    "(repeated_failure=%s, wm_overloaded=%s)",
                    self._repeated_failure, self._wm_overloaded,
                )

            # High frustration: strip high-blast-radius tools
            if self._high_frustration:
                filtered = [
                    t for t in filtered
                    if t.get("function", {}).get("name") not in _RISKY_TOOLS
                ]
                logging.info(
                    "[ATHENA] Cognitive filter: stripping risky tools "
                    "(high_frustration=True)",
                )

            self.tools = filtered
        except Exception:
            logging.debug("[ATHENA] Cognitive tool filter failed — restoring full set")
            self._restore_unfiltered_tools()

    def _restore_unfiltered_tools(self) -> None:
        """Restore the full tool set after cognitive branching."""
        if hasattr(self, "_unfiltered_tools") and self._unfiltered_tools:
            self.tools = list(self._unfiltered_tools)

    def _check_goal_satisfaction(self, messages: list, user_message: str) -> dict:
        assistant_msgs = [m for m in messages[-10:] if m.get("role") == "assistant"]
        if not assistant_msgs:
            return {
                "satisfied": False,
                "reason": "no_assistant_responses",
                "confidence": 0.0,
            }
        ...
        for m in reversed(assistant_msgs):
            if m.get("tool_calls"):
                break
            non_tool_in_a_row += 1
        if non_tool_in_a_row >= 2:
            return {
                "satisfied": True,
                "reason": "llm_stopped_calling_tools",
                "confidence": 0.9,
            }

        last_content = assistant_msgs[-1].get("content", "") or ""
        if len(last_content) > 20 and not assistant_msgs[-1].get("tool_calls"):
            simple_keywords = [
                "what",
                "who",
                "when",
            ...
            first_word = (
                (user_message or "").strip().lower().split()[0]
                if (user_message or "").strip()
                else ""
            )
            if first_word in simple_keywords:
                return {
                    "satisfied": True,
                    "reason": "direct_question_answered",
                    "confidence": 0.7,
                }

        return {"satisfied": False, "reason": "no_heuristic_match", "confidence": 0.0}
    ...
    def run_conversation(
        ...
        self._goal_satisfied = False
        self._low_conf_narrowed = False
        self._wm_overloaded = False
        self._high_frustration = False
        ...
        # Track memory nudge trigger (turn-based + signal-driven, checked here).
        # Phase 2B: signal-driven memory extraction. The old turn-count timer
        # (memory.nudge_interval) is preserved as a fallback baseline, but we
        # ALSO fire memory review when ATHENA's metacognition detected something
        # important: a correction, high surprise/curiosity, or goal completion.
        # The timer still fires (just at a longer interval) as a safety net,
        # while the signal-driven path catches the moments that matter.
        # Skill trigger is checked AFTER the agent loop completes, based on
        # how many tool iterations THIS turn used.
        _should_review_memory = False
        _signal_review = False
        _signal_review_reason = ""
        if hasattr(self, "_cog") and self._cog is not None:
            try:
                cog = getattr(self._cog, "_last_cog", None)
                if cog is not None:
                    meta = cog.get("metacognition", {}) or {}
                    ...
                    for sig in recent_signals or []:
                        sig_name = (
                            sig.get("signal", "") if isinstance(sig, dict) else ""
                        )
                        if sig_name in (
                            "CORRECTED",
                            "GOAL_COMPLETION",
        ...
        # Timer-based fallback (longer interval — 20 turns instead of 10)
        if (
            ...
            if self._turns_since_memory >= max(self._memory_nudge_interval * 2, 20):
                _should_review_memory = True
                self._turns_since_memory = 0

        # Signal-triggered review always fires regardless of timer
        if _signal_review and "memory" in self.valid_tool_names and self._memory_store:
            _should_review_memory = True
            self._turns_since_memory = 0  # reset timer to avoid double-fire
            if not self.quiet_mode:
                self._safe_print(
                    f"  📝 Signal-triggered memory review ({_signal_review_reason})"
                )

        # Add user message
        # 2026-05-18: append a per-turn [Current time: ...] suffix to the
        # LLM-facing content (NOT to user_message itself — keeps persistence
        # / display / search clean). The cached system prompt freezes
        # "Conversation started: <date>" at session-build time and only
        # rebuilds on /new or context compression. For a 24/7 daemon, that
        # cache can be 10+ hours stale (observed: Athena said "Sunday" on
        # Monday morning because her system prompt still said Sunday). This
        # injection lives in the user-message tail, after the prompt-cache
        # boundary, so it costs zero cache hits while giving the model a
        # fresh, authoritative time signal each turn.
        try:
            from hermes_time import now as _hermes_now
            _now_str = _hermes_now().strftime("%A, %B %d, %Y %I:%M %p %Z").rstrip()
            _llm_content = f"{user_message}\n\n[Current time: {_now_str}]"
        except Exception:
            _llm_content = user_message
        user_msg = {"role": "user", "content": _llm_content}
        ...
        # Wall-clock cap (Athena goal-system rethink).
        # On 2026-05-01 a single Telegram round-trip ate 33 min over 30 API
        # calls because max_iterations=90 doesn't notice slow LLM calls. We
        # pull the cap from the cognitive_agent tunables registry when
        # available (so Greg can override via ATHENA_MAX_RESPONSE_SECONDS)
        # and silently fall back to no cap if the import fails — this keeps
        # vanilla Hermes installs unaffected.
        import time as _time_for_cap

        _turn_start_time = _time_for_cap.monotonic()
        _wall_clock_cap_sec = None
        try:
            from cognitive_agent.tunables import get_tunable as _get_tunable
        ...
        while (
            ...
            # Phase 2B: goal-directed early exit
            if getattr(self, "_goal_satisfied", False):
                _turn_exit_reason = "goal_satisfied"
                break

            # Wall-clock cap. Backstop for max_iterations: even if budget
            # is fine, exit cleanly when this turn has gone wall-time over
            # the configured cap. Iteration count is checked at top-of-loop
            # only (we don't kill an in-flight LLM call mid-stream).
            if _wall_clock_cap_sec is not None:
                _elapsed = _time_for_cap.monotonic() - _turn_start_time
                if _elapsed > _wall_clock_cap_sec:
                    _turn_exit_reason = "wall_clock_cap"
                    if not self.quiet_mode:
                        self._safe_print(
                            f"\n⏱  Wall-clock cap hit "
                            f"({_elapsed:.0f}s > {_wall_clock_cap_sec:.0f}s) "
                            f"after {api_call_count} API calls — exiting loop."
                        )
                    break
            ...
            try:
                ...
                _lt_mark(f"iter_start n={api_call_count}")
            except Exception:
                pass
            ...
            # Inject cognitive advisory when signals are active
            if hasattr(self, "_cognitive_advisory") and self._cognitive_advisory:
                effective_system = (
                    effective_system + "\n\n" + self._cognitive_advisory
                ).strip()
            ...
            try:
                ...
                if assistant_message.tool_calls:
                    try:
                        ...
                        _lt_mark(f"tool_dispatch n={api_call_count} tools={_lt_tools}")
                    except Exception:
                        pass
                    if not self.quiet_mode:
                        self._vprint(
                    ...
                    # ── ATHENA: record result to cognitive memory ───────────
                    if self._cog is not None:
                        try:
                            self._cog.record_result(messages, assistant_message)
                        except Exception as _cog_e:
                            logger.debug("[ATHENA] record_result error: %s", _cog_e)

                    # Phase 3A: strategic branching — check cognitive state for multiple signals
                    if self._cog is not None:
                        try:
                            cs = self._cog.cognitive_state
                            advisory_parts = []
                            ...
                            if cs.get("confidence", 0.5) < 0.3:
                                if (
                                    ...
                                    logging.info(
                                        "[ATHENA] Low confidence (%.2f) — narrowing tool options",
                                        cs.get("confidence", 0.5),
                                    )
                            if self._low_conf_narrowed:
                                advisory_parts.append(
                                    f"Low confidence ({cs.get('confidence', 0.0):.0%})"
                                )
                            ...
                            if cs.get("wm_load", 0.0) > 0.8:
                                if not getattr(self, "_wm_overloaded", False):
                                    ...
                                    logging.info(
                                        "[ATHENA] WM overloaded (%.2f) — preferring simple responses",
                                        cs.get("wm_load", 0.0),
                                    )
                            if self._wm_overloaded:
                                advisory_parts.append(
                                    f"WM overloaded ({cs.get('wm_load', 0.0):.0%})"
                                )
                            # High frustration: > 0.7 → avoid risky tool calls, prefer known-good tools
                            if cs.get("frustration", 0.0) > 0.7:
                                if not getattr(self, "_high_frustration", False):
                                    self._high_frustration = True
                                    logging.info(
                                        "[ATHENA] High frustration (%.2f) — avoiding risky actions",
                                        cs.get("frustration", 0.0),
                                    )
                            if self._high_frustration:
                                advisory_parts.append(
                                    f"High frustration ({cs.get('frustration', 0.0):.2f})"
                                )
                            ...
                            if cs.get("failure_rate", 0.0) > 0.6:
                                if not getattr(self, "_high_failure_rate", False):
                                    ...
                                    logging.info(
                                        "[ATHENA] High failure rate (%.2f) — switching strategy",
                                        cs.get("failure_rate", 0.0),
                                    )
                            if self._high_failure_rate:
                                advisory_parts.append(
                                    f"High failure rate ({cs.get('failure_rate', 0.0):.0%})"
                                )
                            # Repeated failure signal → back off immediately
                            _signals = cs.get("metacognitive_signals", [])
                            if any(
                                s.get("signal") == "REPEATED_FAILURE" for s in _signals
                            ):
                                if not getattr(self, "_repeated_failure", False):
                                    self._repeated_failure = True
                                    logging.info(
                                        "[ATHENA] Repeated failure detected — backing off"
                                    )
                            if self._repeated_failure:
                                advisory_parts.append("Repeated failures")

                            # Build advisory string + filter tools if any flags are active
                            if advisory_parts:
                                self._cognitive_advisory = (
                                    f"[COGNITIVE ADVISORY: {'; '.join(advisory_parts)}]"
                                )
                                # Structural enforcement: strip tools based on active signals
                                self._apply_cognitive_tool_filter()
                            else:
                                self._cognitive_advisory = ""
                                # Restore full tool set when no flags are active
                                self._restore_unfiltered_tools()
                        except Exception:
                            pass

                    # ── Phase 2B: goal-directed loop exit ────────────────────
                    if api_call_count >= 3:
                        try:
                            _sat = self._check_goal_satisfaction(
                                messages, original_user_message
                            )
                            if (
                                _sat.get("satisfied")
                                and _sat.get("confidence", 0) >= 0.7
                            ):
                                self._goal_satisfied = True
                                if not self.quiet_mode:
                                    self._safe_print(
                                        f"  ✓ Goal satisfied ({_sat['reason']}, confidence {_sat['confidence']:.0%})"
                                    )
                        except Exception:
                            pass
        ...
        # Wall-clock cap recovery: if we exited because of the wall-clock
        # cap (not iteration budget), ask the model for a one-shot summary
        # the same way budget-exhaustion does. Without this, the user gets
        # an empty reply instead of "here's what I figured out so far."
        if final_response is None and _turn_exit_reason == "wall_clock_cap":
            self._emit_status(
                f"⏱  Wall-clock cap hit ({_wall_clock_cap_sec:.0f}s) "
                f"after {api_call_count} API calls — asking model to summarise"
            )
            if not self.quiet_mode:
                self._safe_print(
        ...
        # ATHENA Phase 3: close cognitive loop at session end
        # record_completion needs full messages + final_response, not available via plugin hook
        try:
            if hasattr(self, "_cog") and self._cog is not None:
                try:
                    from _latency_trace import mark as _lt_mark
                    ...
                    _lt_mark = None
                # ATHENA Phase 14: faithfulness filter — verify final response
                # against the cog state the LLM saw at generation time. In
                # `detect` mode (default) this is telemetry only and returns
                # the original. In strip/regenerate modes it can revise.
                # NEVER raises — passthrough on any internal exception.
                if _lt_mark is not None:
                    try:
                        _lt_mark("verify_start")
                    except Exception:
                        pass
                try:
                    final_response = self._cog.verify_and_revise(final_response)
                except Exception as _fexc:
                    logger.debug("ATHENA verify_and_revise failed: %s", _fexc)
                if _lt_mark is not None:
                    try:
                        _lt_mark("verify_end")
                    except Exception:
                ...
                if _lt_mark is not None:
                    try:
                        _lt_mark("record_start")
                    except Exception:
                        pass
                self._cog.record_completion(
                ...
                # Phase 2A: persist cognitive bridge state for cross-turn continuity
                if self._session_db and self.session_id:
                    try:
                        _bridge = self._cog.bridge_state
                        if _bridge:
                            self._session_db.update_cognitive_state(
                                self.session_id, _bridge
                            )
                    except Exception as _cpse:
                        logger.debug("[ATHENA] Could not persist cog state: %s", _cpse)
        except Exception as exc:
            logger.debug("ATHENA record_completion failed: %s", exc)
